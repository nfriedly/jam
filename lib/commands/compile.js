...
exports.compile = function (opt, callback) {
    ...
    install.initDir(opt.settings, opt.cwd, opt, function (err, opt, cfg, pdir) {
        ...
        // let the jam.requireconfig from the top-level package.json override any config options here
        project.getExtraConfig(function(err, extraconfig) {
        	if (err) {
        		return callback(err);
        	}
        
			_.extend(config, configfile.extraconfig);
