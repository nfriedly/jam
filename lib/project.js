...
exports.updateRequireConfig = function (package_dir, baseurl, /*opt*/rcfg, callback) {
	...
	// todo: use async.parallel for exports.getAllPackages, utils.getJamVersion, and extraconfig.get
    exports.getAllPackages(package_dir, function (err, pkgs) {
        ...
        utils.getJamVersion(function (err, version) {
            ...
            exports.getExtraConfig(function (err, extraConfig) {
            	if (err) {
            		return callback(err);
            	}
				_.extend(data, extraConfig) ;
...
/**
 * Finds the top-leve package.json and returns it's jam.requireconfig or {}
 */
exports.getExtraConfig = function(callback) {
	var packagePath = './package.json';
	pathExists(packagePath, function(exists) {
		if (exists) {
			// todo: look at util.js#readPackageJSON and maybe use it
			utils.readJSON(packagePath, function (err, package) {
				if (err) {
					return callback(err);
				}
				if(package && package.jam && package.jam.requireconfig) {
					callback(null, package.jam.requireconfig);
				} else {
					callback (null, {});
				}
			});
